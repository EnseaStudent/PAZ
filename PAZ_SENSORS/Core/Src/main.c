...
/* USER CODE BEGIN PV */
		...
		#define SAMPLING_PERIOD   50  // en millisecondes
...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	bool heartbeatDetected(uint32_t adc_port, uint32_t delay) {
	    static uint32_t maxValue = 0;
  ...
  while (1)
  {
	  if (heartbeatDetected(ADC_CHANNEL_0, 60))
	  {
		  printf("bool = 1 \r\n");
		  HAL_Delay(500);
	  }
